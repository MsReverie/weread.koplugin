...
-- On overflow the whole map is dropped; the next tap simply re-reads SQLite.
local THOUGHT_PAGE_CACHE_MAX = 300  -- distinct tapped underlines
...
function WeReadPlugin:_showThoughtPopup(pages, link, session_gen, tap_started)
...
function WeReadPlugin:_ensureThoughtDB(book_id)
...
function WeReadPlugin:_buildThoughtPagesFromHref(href)
...
function WeReadPlugin:_queueThoughtPopup(pages, link, tap_started)
    if type(pages) ~= "table" or #pages == 0 then
        return true
    end
...
function WeReadPlugin:_downloadMissingThought(info, href, link, tap_started)
    if self._thought_refresh_request then
        return true
    end
    if not self:requireLogin(false, true) then
